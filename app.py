...
def submit():
    ...
    client = Client()
    prompt = f"Здравствуйте техническая поддержка телеграмма! Я наткнулся на пользователя с никнеймом: {nickname} он нарушает такое правило в Telegram как: {complaint_type}. Таким образом он мешает окружающим пользователям, нарушая правила ToS Telegram, пожалуйста, примите меры против этого пользователя и заблокируйте его аккаунт, заранее спасибо, С уважением, Артур"
