...
# Токен бота
TOKEN = '7520829818:AAG0Xo_J3Z1jesbor36Q9Z92P6uCO-zIzGM'

# Список ID администраторов
ADMIN_IDS = [7601411293]  # Замените на ваши ID
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        user_status = await context.bot.get_chat_member(chat_id='@xcxdgfg', user_id=user_id)
        if user_status.status not in ['member', 'administrator', 'creator']:
            await update.message.reply_text('❌ Пожалуйста, подпишитесь на канал @xcxdgfg, чтобы пользоваться ботом.')
            return
